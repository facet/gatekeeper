...
AuthAPI.prototype.apiAuth = function(query, nodeStack) {
  if(typeof query === 'undefined' || typeof query === null || !query.hasOwnProperty('conditions')) {
    ...
    // TOOD: return false? or better yet is probably nodeStack.next(new Error(...))
  }
  ...
  // look up user
  this.intercom.emit('facet:user:findone', query, nodeStack, successCb, errorCb);
};
