...
var AuthAPI = function( options ) {
  ...
  AuthAPI.super_.call(this, options);
}
 ...
 * 
 * @param  {object} query      contains the request variables, is constructed by the 
 *                             corresponding CRUD function in FacetApiCore
 * @param  {object} nodeStack  Contains keys for req, res, next
 *
 * @return  string
 */
AuthAPI.prototype.loginJwt = function(query, nodeStack) {
  ...
  // test password
  this.intercom.emit('facet:user:findone:auth', userQuery, function jwtLoginSuccess(user) {
...
AuthAPI.prototype.loginAccount = function(query, nodeStack) {
  ...
  // test password
  this.intercom.emit('facet:user:findone:auth', userQuery, function accountLoginSuccess(user) {
  ...
  if (token) {
    try {
      ...
      if (decoded.exp <= Date.now()) {
        this.intercom.emit('facet:response:error', 400, 'API token expired, please login again for a new one.');
        return new Error('API token expired, please login again for a new one.');
      }
      ...
      this.intercom.emit('facet:user:findone', userQuery, function accountLoginSuccess(user) {
        ...
        _this.intercom.emit('facet:auth:api:done', user, nodeStack);
...
AuthAPI.prototype.apiAuthBasic = function(query, nodeStack) {
  ...
  var successCb = function apiAuthSuccess(apiUser) {
    // console.log('finished api basic auth, emitting facet:auth:api:done');
  ...
  // look up user
  // console.log('about to look up user in apiAuthBasic... ');

  this.intercom.emit('facet:user:findone:auth', query, successCb, errorCb);
};
