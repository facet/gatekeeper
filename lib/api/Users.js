 ...
 * @type   {Object}
 */
UsersAPI.prototype.routerManifest = {
  route: '/users',  // this can be overwritten
  routes: [
    { verb: 'GET',    route: '/:userId',  emit: 'facet:user:find'   },  // GET a single User by id
    ...
    { verb: 'DELETE', route: '/:userId',  emit: 'facet:user:delete' },  // DELETE a single resource
  ]
};
 ...
 * Finds User documents with requested fields based on query and options
 * 
 * @param   {Object}    query       Contains keys for: conditions, fields, options
...
UsersAPI.prototype.findOne = function(query, successCb, errorCb) {
  ...
  var queryBuilder = this.User.findOne(query.conditions, query.fields, query.options);

  console.log('right before adding populate: ', query.populate);
  console.log(_.isEmpty(query.populate));

  if( !_.isEmpty(query.populate) ) {
    console.log('wtf');
    if( _.isArray(query.populate) ) {
      for (var i = query.populate.length - 1; i >= 0; i--) {
        console.log('adding populate: ', query.populate[i]);
        queryBuilder.populate(query.populate[i]);
      };
    }
    else {
      console.log('adding populate: ', query.populate);
