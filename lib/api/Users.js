...
UsersAPI.prototype.setupRouterManifest = function () {
  ...
  this.routerManifest
    ...
    .setRouteBase('/users')
    .setRoutes([
      { verb: 'GET',    route: '/:userId', emit: 'facet:user:findone',  processor: true },  // GET a single user by id
      { verb: 'GET',    route: '',         emit: 'facet:user:find',     processor: true },  // GET an array of user objects 
      { verb: 'POST',   route: '',         emit: 'facet:user:create',   processor: true },  // POST new user
      { verb: 'PUT',    route: '/:userId', emit: 'facet:user:update',   processor: true },  // PUT single/multiple users
      { verb: 'DELETE', route: '/:userId', emit: 'facet:user:remove',   processor: true },  // DELETE a single user resource
    ])
...
UsersAPI.prototype.registerEvents = function () {
  ...
  this.intercom.on('facet:user:test:event', this.testHandler.bind(this) );

};


UsersAPI.prototype.testHandler = function(query) {
  console.log('=====  Extended route bound correctly!!!! ', query);
}
