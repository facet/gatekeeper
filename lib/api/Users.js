...
UsersAPI.prototype.apiAuth = function(query, nodeStack) {
	if(typeof query === 'undefined' || typeof query === null || !query.hasOwnProperty('conditions')) {
		...
		console.log('No API credentials were specified.');
		return false;
		...
		this.intercom.emit('facet:response:error', response);
	}
	...
	this.User.findOne(query.conditions, '', query.options)
		...
		.then(function(apiUser) {
			_this.intercom.emit('facet:response:user:auth:api', apiUser, nodeStack);
		},
		function(err) {
			// TODO: emit 403 error
			var error = {
