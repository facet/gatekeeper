...
function UserSchema(options, BaseSchema){
  ...
  UserSchema.pre('save', function(next) {
    console.log(this.api_access, typeof this.api_access);

    if( this.api_access === true ) {
      var unique = uuid.v4();
      this.api_key = crypto.createHash('sha256').update(unique).digest('hex');
    }
    next();
  });
  ...
  UserSchema.methods.checkAccess = function(action, cb, app_id) {
    ...
    function respond() {
      ...
      if( _.isEmpty(_this.app_id) && app_id === 'global' ) {
        ...
        else if( _this.groupPermsCascaded['global'] && !_.isUndefined(_this.groupPermsCascaded['global'][action]) && _this.groupPermsCascaded['global'][action] !== 0 ){
          cb(_this.levelToBool(_this.groupPermsCascaded['global'][action]));
          return;
        }
        else if( _this.groupPermsCascaded['global'] && !_.isUndefined(_this.groupPermsCascaded['global']['*']) ) {
