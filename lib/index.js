 ...
 */
util.inherits(GateKeeper, ApiCore);
 ...
 * @return  function
 */
GateKeeper.prototype.apiAuthMiddleware = function(type) {

  var _this = this;
  this.apiAuthInitialized = true;

  if( this.doAccessCheck === true ) {
    if( _.isFunction(type) ) {
      ...
      if( type === 'express' || !type ) type = 'connect';

      var apiAuthFn = require('./middleware/apiauth/' + type);
      return apiAuthFn.bind(this);
    }
  }
  else {
    return this.noop;
